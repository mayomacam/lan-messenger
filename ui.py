    ...
    def open_private_chat(self, ip, name):
        ...
        if not tab_name:
            ...
            def send_priv(e=None, i=ip, ent=entry):
                ...
                if not m: return
                ttl = self.get_ttl_seconds()
                mid = self.db.add_message(self.username, m, recipient=i, ttl=ttl)
                threading.Thread(target=self.network.send_message, args=(i, self.username, m, mid, True, ttl)).start()
                ent.delete(0, "end")
