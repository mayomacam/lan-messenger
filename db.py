...
class Database:
    ...
    def get_peer_permissions(self, ip: str) -> dict:
        ...
        return self.get_peers_permissions([ip]).get(ip, {
            'is_blocked': 0,
            'can_chat': 1,
            'can_list_files': 1,
    ...
    def get_peers_permissions(self, ips: List[str]) -> dict:
        ...
        with self.lock:
            cursor = self.conn.execute(f"""
                SELECT ip, can_chat, can_list_files, can_download_files, is_blocked
                FROM trusted_peers
        ...
        # Ensure every requested IP has a result (even if default)
        for ip in ips:
            if ip not in result:
                result[ip] = {'can_chat': True, 'can_list_files': True, 'can_download_files': True, 'is_blocked': False}
        return result
        ...
        with self.lock:
            with self.conn:
                self.conn.execute("""
                    ...
                    WHERE ip = ?
                """, (
                    int(perms.get('is_blocked', 0)),
                    int(perms.get('can_chat', 1)),
                    int(perms.get('can_list_files', 1)),
                    int(perms.get('can_download_files', 1)),
                    ip
                ))
