...
class Database:
    ...
    def get_trusted_peer(self, ip: str) -> Tuple:
        with self.lock:
            ...
            cursor = self.conn.execute("""
                SELECT ip, username, fingerprint, trust_level, last_seen,
                       can_chat, can_list_files, can_download_files, is_blocked
                FROM trusted_peers WHERE ip = ?
            """, (ip,))
            return cursor.fetchone()

    def update_peer_permissions(self, ip: str, can_chat: bool, can_list: bool, can_download: bool, is_blocked: bool):
        with self.lock:
            with self.conn:
                self.conn.execute("""
                    ...
                    WHERE ip = ?
                """, (can_chat, can_list, can_download, is_blocked, ip))
